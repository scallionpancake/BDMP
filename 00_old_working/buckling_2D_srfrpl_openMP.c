/***** Brownian Dynamic Simulation of 2D Buckling *****/
...
#include "vector.h"
#include "utility.h"
#include "buckling_2D_srfrpl_openMP_global.h"
...
int main(int argc, char **argv) {
    ...
    while (t < tmax) {
        ...
        if (t >= ti + trex) {
            // rotational field by orthogonal cos an sin wave with pi/2 phase diff
            VSet2(H0, 0, Ht);
        } else if (t < ti) {
            // rotational field by orthogonal cos an sin wave with pi/2 phase diff
            VSet2(H0, Ht, 0);
            ...
            // rotational field by orthogonal cos an sin wave with pi/2 phase diff
            VSet2(H0, (Ht*(1-(t-ti)/trex)), (Ht*((t-ti)/trex)));
        }
        VSCopy2(Mdp, Mdpt, H0);
        Mdp2 = VDot2(Mdp,Mdp);
